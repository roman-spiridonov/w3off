...
from signer import vault
from signer.vault import (
    decrypt_pkey,
    decrypt_str,
    encrypt_pkey,
    encrypt_str,
...
from signer.w3signer import dictToSignedTx, signTx, signedTxToDict


class Test_Signer(unittest.TestCase):
    def test_signTx(self):
        import test.data.txTestAaveSupply as t

        with open(os.path.join("test", "data", "keystore-test.json")) as f:
...
class Test_Vault(unittest.TestCase):
    ...
    def test_pkey_from_keystore(self):
        with open(os.path.join("test", "data", "keystore-test2.json")) as f:
        ...
        with open(os.path.join("test", "data", "keystore-default.json")) as f:
            keystore2 = json.load(f)
