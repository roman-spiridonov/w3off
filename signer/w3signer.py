...
def run_signer(tx, **pparams):
    ...
    while not isOffline:
        print(
            f"{cli_helpers.bcolors.WARNING}You are not offline!{cli_helpers.bcolors.ENDC} Make sure you disconnect your network (and VPN tunnel, if one is available)."
        )
    ...
    if isOffline:
        print(f"{cli_helpers.bcolors.OKGREEN}Good!{cli_helpers.bcolors.ENDC} You are offline, so we can proceed.")
